...
// Refs for current mode — click handlers on markers are closures created once,
// so they need refs to read the latest values
const drawModeRef = useRef(drawMode);
const addLocationModeRef = useRef(addLocationMode);
const onNodeClickRef = useRef(onNodeClick);
useEffect(() => { drawModeRef.current = drawMode; }, [drawMode]);
useEffect(() => { addLocationModeRef.current = addLocationMode; }, [addLocationMode]);
...
map.on("load", () => {
  // Terrain source for elevation data but NO exaggeration — prevents marker drift
  if (!map.getSource("mapbox-dem")) {
    ...
    // Intentionally NOT calling map.setTerrain() — 3D terrain causes markers to drift on zoom
  }
});
...
nodes.filter(n => n.id !== selNode).forEach(n => {
  ...
  // Distance label as a Mapbox marker (follows pan/zoom automatically)
...
nodes.forEach(node => {
  ...
  if (markersRef.current[node.id]) {
    ...
    el.addEventListener("click", (e) => {
      e.stopPropagation();
      if (!drawModeRef.current && !addLocationModeRef.current) onNodeClickRef.current(node.id);
    });
...
```
  ...
  {/* Distance labels are now Mapbox markers — no static divs needed */}
